...
Page({
  ...
  onLoad: function (options) {
    ...
    //点击切换导航的回调
    changenav(event){
      let navId=event.currentTarget.id;//item.id string类型 通过id event传值的时候会把Number转换成string
      this.setData({
        navId:navId*1,  //将string转化成Number
        // navId>>>0  位移运算符 将目标数据转换为二进制 然后移动指定的位数  右移两位会强制转换成number类型
        // 动态获取当前导航对应的视频数据
        videoList:[]
      })
      wx.showLoading({
    ...
    handleTimeUpdate(event){
      ...
      let {videoUpdateTime}=this.data;
      // 判断记录播放时长的数组videoupdatetime中是否有当前的播放记录  如果有 在原有的记录中修改当前的播放时间就可以 如果没有就需要添加当前播放视频的对象
  ...
  onShareAppMessage: function ({from}) {
    if(from==='button'){
      return{
        ...
        imageUrl:'/static/images/benjamin.jpg'
      }
    }else{
      return{
        title:'跟我一起听音乐',
